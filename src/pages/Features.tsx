...
export const  Features = () => {
    return (
        <main>
                  {/* SECTION (Info Cards) */}
      <section className=" grid grid-cols-1 bg-gray-50 text-gray-900 p-12">
          {/*Container*/}
          ...
          <div className="flex flex-col items-center gap-8 w-full max-w-2xl mx-auto">
            {person.infoCards.map((infoCard) => (
              <div key={infoCard.title} className="snap-center w-full p-6 bg-white  rounded-xl shadow-lg border border-gray-100 
               transition-transform hover:scale-105 duration-300 ease-in-out
               flex flex-col items-center justify-center text-center">
                <span className="text-xs  text-gray-900 font-bold  uppercase tracking-widest mb-2">
                  {infoCard.title}
                </span>
